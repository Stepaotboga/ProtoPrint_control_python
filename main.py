...
CORNER_POINT = (98.0, 275.0) # угол закрепления заготовки
...
if not USE_BOUNDS:
    GCODE_WIDTH = 60.0      # X axis
    GCODE_HEIGHT = 60.0     # Y axis
...
class PCBHeightMapper:
    ...
    def process_gcode_file(self, input_file, output_file):
        ...
        # Обработка G-code
        interpolator.ZERO_POINT = (zero_point[0], zero_point[1], 5, 0)
        if USE_EXTRUDER:
        ...
        if not USE_EXTRUDER:
            ...
            shift_z = self.z_offset * -1 - 0.4 + 0.05
            shift_xy = (OFFSET_EXTRUDER[0], OFFSET_EXTRUDER[1])
            processed_lines = gcode_analysis.added_xy_shift(processed_lines, shift_xy)
